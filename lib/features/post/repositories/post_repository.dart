...
class PostRepository {
  ...
  CollectionReference get _nestedComments =>
      _firestore.collection(FirebaseConstants.nestedCommentsCollection);
  ...
  FutureVoid replyToComment(NestedComment comment) async {
    try {
      return right(_nestedComments.doc(comment.commentId).set(comment.toMap()));
    } on FirebaseException catch (e) {
      throw e.message.toString();
    } catch (e) {
      return left(Failure(e.toString()));
  ...
  Stream<List<NestedComment>> getNestedComments(String parentId) {
    return _nestedComments
        .where('parentId', isEqualTo: parentId)
        .orderBy('createdAt', descending: true)
        .snapshots()
        .map((event) => event.docs
            .map((e) => NestedComment.fromMap(e.data() as Map<String, dynamic>))
            .toList());
  }
